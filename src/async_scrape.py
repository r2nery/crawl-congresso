                ...
                try:
                    ...
                    if itens > 1:
                        ...
                        for retry in range(10):
                            try:
                                tqdm.write(f"🔁 Final retries at itens=1 (try {retry+1}/10) for dep {dep_id}, leg {leg}, page {pagina}")
                                page_data = await fetch_page(session, dep_id, leg, pagina, itens, params_base, pbar=pbar)
                                if not page_data:
                                    success = True
                                    break
                                ...
                                delay = 2 ** retry
                                tqdm.write(f"⏳ Retry {retry+1}/10 failed for dep {dep_id}, leg {leg}, page {pagina}. Waiting {delay}s. Error: {type(e).__name__}")
                                await asyncio.sleep(delay)
                        if not success:
                            tqdm.write(f"❌ Giving up on dep {dep_id}, leg {leg}, page {pagina} after 3 retries at itens=1")
                            break
...
def get_unique_deputado_ids(filepath='./data/raw/deputados.csv'):
